  ...
  return (
    <div className="rounded-2xl border border-[#2A2238] bg-[#1D1829]/60 p-4 transition-colors focus-within:border-[#3A3050]">
      <div className="flex items-center justify-between">
        ...
        {hint && (
          <span className="text-[10px] text-[#9088A0]/60">
            {hint}
          </span>
        )}
      </div>
...
export function ProfileEditPage() {
   ...
   *
   * If the user came from /home/me, router.back()
   * will normally take them there.
   *
   * Otherwise fall back to /home/me.
   */
  const handleClose = () => {
    if (
   ...
   *
   * IMPORTANT:
   * After a successful save we go directly to /home/me.
   *
   * We do NOT use /profile anymore.
   */
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
     ...
     *
     * This keeps the PATCH request minimal and prevents
     * accidentally overwriting values that weren't edited.
     */
    ...
    if (
      ...
      /*
       * Do not send an empty avatar.
       *
       * The current UI only supports replacing an avatar,
       * not deleting it.
       */
      if (form.avatar.trim() !== "") {
    ...
    if (
      form.country_flag.trim() !==
      (profile.country_flag ?? "")
    ) {
      payload.country_flag =
        form.country_flag.trim();
    }
     ...
     * Nothing changed.
     *
     * Still go back to the profile page because
     * the user explicitly pressed Save Changes.
     */
    ...
    try {
      ...
      /*
       * Keep local state synchronized in case this
       * component remains mounted briefly during navigation.
       */
       ...
       * router.push("/profile")
       *
       * That route is not the requested destination.
       *
      ...
      /*
       * Handle field-level validation errors if apiFetch
       * exposes them as `details`.
       */
      const maybeDetails = (
        err as
          | {
      ...
      if (maybeDetails) {
        ...
        for (const [
          key,
          messages,
        ] of Object.entries(maybeDetails)) {
          if (messages?.[0]) {
            flat[key] = messages[0];
          }
        }
        ...
        >
          ...
          <div className="flex flex-col items-center gap-3 py-2">
            <div className="relative">
              <div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-full border-2 border-[#2A2238] bg-[#2A2238] shadow-[0_0_0_4px_rgba(203,163,92,0.08)]">
                {form.avatar ? (
                  ...
                  <img
                    ...
                    onError={(event) => {
                      /*
                       * Don't crash the page if the uploaded URL
                       * cannot be loaded.
                       */
          ...
          >
            <div className="mt-1 flex items-center gap-1 rounded-lg border border-[#2A2238] bg-[#17131F] px-3 py-2.5">
              <span className="text-sm text-[#9088A0]">
                @
              </span>
          ...
          {/* Gender */}
          <div>
            <p className="mb-2 text-xs font-medium text-[#9088A0]">
              Gender
            </p>
