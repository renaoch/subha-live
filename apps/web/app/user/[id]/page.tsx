...
const fallback: PublicProfile = {
  ...
  gender: null, level: 6, vip_level: 0, svip: false, is_verified: true, followers: 12400, following: 128, friend_count: 0, role: "user", created_at: new Date().toISOString(),
};
    ...
    <main className="min-h-dvh bg-[#080808] text-white">
      <div className="mx-auto min-h-dvh max-w-2xl border-x border-white/[0.06] bg-[#0c0c0c]">
        ...
        <section className="border-b border-white/[0.07] px-6 pb-7 pt-8">
          ...
          <div className="mt-6 grid grid-cols-3 divide-x divide-white/[0.08] text-center"><Stat value={profile.followers} label="Followers" onClick={() => setTab("followers")} /><Stat value={profile.following} label="Following" onClick={() => setTab("following")} /><Stat value={profile.friend_count ?? friends.length ?? 0} label="Friends" onClick={() => setTab("friends")} /></div>
        </section>
